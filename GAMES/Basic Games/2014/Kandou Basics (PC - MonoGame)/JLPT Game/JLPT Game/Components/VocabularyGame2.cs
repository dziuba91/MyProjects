...
{
	class VocabularyGame2 : DrawableGameComponent
	{
		#region Field
		...
		public VocabularyGame2(Game game)
			: base(game)
		{
			random = new Random();
		...
		public void Draw(GameTime gameTime, MouseState d, Vector2 position)
		{
			if (!pauseGame)
			{
				string test1 = composition[CorectKanji].meaning;
				...
				for (int i = 0; i < 6; i++)
								...
								(position.Y >= choose[i].Y && position.Y <= choose[i].Y + choose[i].Height)))
					{
						if (d.LeftButton == ButtonState.Pressed)
						{
							...
							spriteBatch.DrawString(text6, composition[kanjiIndex[i]].reading, new Vector2(choose[i].X + (choose[i].Width / 2) - (text6.MeasureString(composition[kanjiIndex[i]].reading).X / 2), choose[i].Y + (choose[i].Height / 2) - (text6.MeasureString(composition[kanjiIndex[i]].reading).Y / 2)), Color.Black);
							pressed = true;
						}
						else if (pressed)
						{
						...
						else
						{
							...
							spriteBatch.DrawString(text6, composition[kanjiIndex[i]].reading, new Vector2(choose[i].X + (choose[i].Width / 2) - (text6.MeasureString(composition[kanjiIndex[i]].reading).X / 2), choose[i].Y + (choose[i].Height / 2) - (text6.MeasureString(composition[kanjiIndex[i]].reading).Y / 2)), Color.White);
						}
					}
					else
					{
