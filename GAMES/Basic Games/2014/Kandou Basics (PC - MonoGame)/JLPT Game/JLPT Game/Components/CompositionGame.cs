...
{
	class CompositionGame : DrawableGameComponent
	{
		#region Field
		...
		~CompositionGame() 
		{
			...
			Game.Components.Remove(this.compList);
			compList.Dispose();
			compList = null;
		}
		...
		protected override void LoadContent()
		{
			...
			for (int i = 0; i < kanji.Length; i++)
			{
				for (int j = 0; j < kanji[i].submissions.Length; j++)
				{
					if (kanji[i].submissions[j].priority >= minPoints)
					{
						composition[index] = kanji[i].submissions[j];
			...
			compList.Composition = composition;
			compList.CompositionIndex = compositionIndex;
			compList.CompMaxIndex = index;
		...
		public void Draw(GameTime gameTime, MouseState d, Vector2 position)
		{
			if (!pauseGame && !pauseGame2)
			{
				...
				string test2 = "";
				if(showReading) test2 = composition[CorectKanji].reading;
			...
			else
			{
				...
				else if (pauseGame2) compList.Draw(gameTime, d, position);
		...
		private void compositionListButton(MouseState d, Vector2 position)
		{
			spriteBatch.Begin();
